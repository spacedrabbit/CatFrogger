...
class GameLevel: CustomStringConvertible {
  ...
  func gridColumnAndRowAfterMoveInDirection(direction: MoveDirection, currentGridColumn: Int, currentGridRow: Int) -> (didMove: Bool, newGridColumn: Int, newGridRow: Int) {
    ...
    let type = gameLevelDataTypeForGridPosition(newGridColumn, row: newGridRow)
  ...
  func setupLevelAtPosition(position: SCNVector3, parentNode: SCNNode) {
    ...
    for row in 0..<data.rowCount() {
      ...
      let type = gameLevelDataTypeForGridPosition(5, row: row)
      switch type {
      case GameLevelDataType.Road:
        ...
        roadNode.position = coordinatesForGridPosition(Int(data.columnCount() / 2), row: row)
        ...
        let _ : Float = row % 2 == 0 ? 1.0 : -1.0
        
        // Determine the position of the node
        var position = coordinatesForGridPosition(startCol, row: row)
      ...
      default:
        ...
        grassNode.position = coordinatesForGridPosition(Int(data.columnCount() / 2), row: row)
        grassNode.rotation = SCNVector4(x: 1.0, y: 0.0, z: 0.0, w: -3.1415 / 2.0)
        levelNode.addChildNode(grassNode)
        
        // Create obstacles
        for col in 0..<data.columnCount() {
          let subType = gameLevelDataTypeForGridPosition(col, row: row)
          if subType == GameLevelDataType.Obstacle {
            ...
            let gridPosition = coordinatesForGridPosition(col, row: row)
